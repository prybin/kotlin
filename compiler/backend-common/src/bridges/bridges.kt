...
public fun <Function : FunctionHandle, Signature> generateBridges(
    ...
    if (fake) {
        // If it's a concrete fake override, some of the bridges may be inherited from the super-classes.
        // Specifically, consider all concrete functions such that the path from the given function to them consists only of concrete
        // functions. Some bridges are already generated for those vertices and will be inherited here. Note that all such bridges are
        // guaranteed to delegate to the same implementation as bridges for the current vertex, that's why it's safe to inherit them
        val handler = object : DFS.AbstractNodeHandler<Function, Unit>() {
            override fun afterChildren(current: Function?) {
                if (current != null && current != function) {
                    bridgesToGenerate.removeAll(findAllReachableDeclarations(current).map(signature))
                }
            }
        ...
        DFS.dfs(listOf(function), { it!!.getOverridden().filter { !it.isAbstract } as Iterable<Function> }, handler)
    }
