...
public class IdeErrorMessages {
    ...
    static {
        ...
        MAP.put(CONFLICTING_OVERLOADS, "<html>''{0}''<br />is already defined in {1}</html>",
                DescriptorRenderer.HTML_COMPACT_WITH_MODIFIERS, HTML_TO_STRING);

        MAP.put(RESULT_TYPE_MISMATCH, "<html>Function return type mismatch." +
                                      "<table><tr><td>Expected:</td><td>{1}</td></tr>" +
                                      "<tr><td>Found:</td><td>{2}</td></tr></table></html>", HTML_TO_STRING, HTML_RENDER_TYPE, HTML_RENDER_TYPE);
        ...
        MAP.put(DELEGATE_SPECIAL_FUNCTION_AMBIGUITY, "<html>Overload resolution ambiguity on method ''{0}''. All these functions match. <ul>{1}</ul></html>", HTML_TO_STRING, HTML_AMBIGUOUS_CALLS);
        MAP.put(DELEGATE_SPECIAL_FUNCTION_NONE_APPLICABLE, "<html>Property delegate must have a ''{0}'' method. None of the following functions is suitable. <ul>{1}</ul></html>",
                HTML_TO_STRING, HTML_NONE_APPLICABLE_CALLS);
